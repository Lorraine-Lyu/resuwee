    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes)
        ...
        if (qName.equals("node")) {
            ...
            // System.out.println("Node id in this way: " + attributes.getValue("ref"));
                ...
                //need to check the tag firs before adding
            road.add(attributes.getValue("ref"));
        ...
        if (qName.equals("way")) {
            if (flag) {
                for (int i = 0; i < road.size(); i++) {
                    if (i > 0) {
                        g.nodes.get(road.get(i)).to.add(road.get(i - 1));
                        g.nodes.get(road.get(i - 1)).to.add(road.get(i));
                    }
                }
                flag = false;
            }
